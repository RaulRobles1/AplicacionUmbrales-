...
@section('contenido')
   ...
   /* Estilos de la tabla
    ...
    <table class="tabla-hidro">
        ...
        <tbody>
            {{-- Recorre todos los Marcos de Control --}}
            ...
            @foreach ($roeas as $mc)
                @php
                    ...
                    if ($mc->ur_ccaa_influencia) {
                        $ids = explode(',', $mc->ur_ccaa_influencia);
                        $nombresEncontrados = [];
                        foreach ($ids as $id) {
                            $idLimpio = trim($id);
                ...
                <tr class="fila-alerta-{{ $mc->nivel_alerta }}" style="{{ $mc->ur_activo ? '' : 'opacity: 0.6;' }}">
                    <td style="text-align: center;">
                        {{ $mc->ur_activo ? 'ACTIVO' : 'NO ACTIVO' }}
                    </td>

                    <td>
                        <strong>{{ $mc->ur_codigo }}</strong><br>
                        {{ $mc->ur_nombre }}
                    </td>
                    {{-- Si no encuentra el dato pone "---" para que no pete --}}

                    <td>
                        <small><b>Río:</b> {{ $mc->ur_rio ?? '---' }}</small><br>
                        <small class="text-muted"><b>Zona:</b> {{ $mc->ur_zona_explotacion ?? '---' }}</small>
                    </td>

                    <td>{{ $mc->ur_tag_ip21 ?? '---' }}</td>
                    <td>{{ $mc->ur_tag_ip21_caudal ?? '---' }}</td>
                    <td>{{ $mc->ur_tag_digital_ip21 ?? '---' }}</td>

                    {{-- Solo se ven 2 decimales y punto para miles --}}

                    <td style="font-weight: bold;">
                        {{ $mc->rde_valor ? number_format($mc->rde_valor, 2, ',', '.') : '---' }}
                    </td>
                    <td>{{ $mc->rde_hora ?? 'No hay hora' }}</td>
                    {{-- ULTIMO NIVEL DE ALERTA --}}

                    <td>
                        <span class="status-dot dot-{{ $mc->ur_ultimo_nivel_alerta ?? 0 }}"></span>
                        @if ($mc->ur_ultimo_nivel_alerta == 3)
                            <span style="color:red">ALERTA 3</span>
                        @elseif($mc->ur_ultimo_nivel_alerta == 2)
                            <span style="color:orange">ALERTA 2</span>
                        @elseif($mc->ur_ultimo_nivel_alerta == 1)
                    ...
                    {{-- ESTADO ACTUAL --}}
                    <td>
                        <span class="status-dot dot-{{ $mc->nivel_alerta }}"></span>
                        @if ($mc->nivel_alerta == 3)
                            <b style="color:red">ALERTA 3</b>
                        @elseif($mc->nivel_alerta == 2)
                            <b style="color:orange">ALERTA 2</b>
                        @elseif($mc->nivel_alerta == 1)
                    ...
                    <td>
                        <div class="grafico-fila" data-codigo="{{ $mc->ur_codigo }}" style="width: 150px; height: 40px;">
